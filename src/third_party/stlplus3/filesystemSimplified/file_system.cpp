...
#define S_ISSOCK(mode) (false)
    ...
    bool is_file_std(const std::string& thing)
    {
        try {
            // Create a path object (automatically handles trailing separators)
            fs::path path(thing);
            ...
            if (!fs::is_regular_file(path, ec) && !fs::is_symlink(path, ec)) {
                ...
                // If there was no error, but it's not a regular file or symlink,
                // check if it's one of the other file-like types
                if (!(fs::is_socket(path) || fs::is_fifo(path))) {
                    return false;
                }
            }
    ...
    void folder_files2(const std::string& directory, std::vector<std::string>& files)
    {
        std::vector<std::string> files2 = folder_wildcard(directory, "*", false, true);
